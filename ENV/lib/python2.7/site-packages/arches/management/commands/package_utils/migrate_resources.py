...
def migrate(settings=None):
                    ...
                    for group_root_node_ids, group_migrations in groups:
                        ...
                        for group_root_entity_model in group_root_entities:
                            ...
                            try:
                                ...
                                errors.append("Error saving %s. Error was %s" % (resource.id, e))
    ...
    if len(errors) > 0:
        utils.write_to_file(os.path.join(settings.PACKAGE_ROOT, 'logs', 'migration_errors.txt'), '\n'.join(errors))
        print "\n\nERROR: There were errors migrating some resources. See logs/migration_errors.txt"
        if break_on_error:
            sys.exit(101)
    ...
    try:
        while typeid != resource_entity_type_id:
            try:
                ...
                relationship = models.Relations.objects.all().filter(entityidrange=parent.entityid)[0]
