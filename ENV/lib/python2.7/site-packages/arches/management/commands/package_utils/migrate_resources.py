...
def migrate(settings=None):
    ...
    logging.warning("GC enabled: %s", gc.isenabled())
            ...
            for f in listdir(path):
                if isfile(join(path, f)) and f.endswith(suffix):
                    ...
                    for group_root_node_ids, group_migrations in groups:
                        ...
                        group_root_entities = models.Entities.objects.filter(entitytypeid=group_root_node_id).iterator()
                        ...
                        for group_root_entity_model in group_root_entities:
                            ...
                            # free resources
                            logging.warning("Collecting garbage")
                            logging.warning("Counts: %s", gc.get_count())
...
def insert_actors(settings=None):
    ...
    for entry in settings.ACTOR_NODES:
        ...
        parent_entities = models.Entities.objects.filter(entitytypeid=parent_entitytypeid).i
        ...
        if os.path.exists(path):
            for f in listdir(path):
                if isfile(join(path,f)) and f.endswith(suffix):
                    ...
                    entities_to_delete = models.Entities.objects.filter(entitytypeid__in=all_entitytypeids_to_remove)
                    ...
                    # dates
                    models.Dates.objects.filter(entityid__in=entities_to_delete).delete()
                    # files
                    models.Files.objects.filter(entityid__in=entities_to_delete).delete()
                    # strings
                    models.Strings.objects.filter(entityid__in=entities_to_delete).delete()
                    # geometries
                    models.Geometries.objects.filter(entityid__in=entities_to_delete).delete()
                    # numbers
                    models.Numbers.objects.filter(entityid__in=entities_to_delete).delete()
                    # domains
                    models.Domains.objects.filter(entityid__in=entities_to_delete).delete()
                    # delete any relationships from or to this entity id
                    models.Relations.objects.filter( Q(entityiddomain__in=entities_to_delete) | Q(entityidrange__in=entities_to_delete) ).delete()
                    ...
                    models.Mappings.objects.filter(entitytypeidto__in=all_entitytypeids_to_remove).delete()
                    
                    # find rules involving this entity type
                    rules = models.Rules.objects.filter( Q(entitytypedomain__in=all_entitytypeids_to_remove) | Q(entitytyperange__in=all_entitytypeids_to_remove))
                    # remove mapping steps associated to these rules
                    models.MappingSteps.objects.filter(ruleid__in=rules).delete()
                    # remove the rules
                    rules.delete()
