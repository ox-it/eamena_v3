            ...
            for f in listdir(path):
                if isfile(join(path,f)) and f.endswith(suffix):
                    ...
                    for concept_model in concepts_to_delete:
                        # if it isn't referenced by another entitytypeid
                        references = models.EntityTypes.objects.all().filter(conceptid=concept_model)
                        ...
                        if len(references) <= 1:
                            ...
                            logging.warning("concept already in use for %s", concept_model.legacyoid)
...
def convert_resource(resourceid, target_entitytypeid):
    ...
    # traverse the entire entity tree
    # for each node found:
        # convert the relation to the new rule
    pass
