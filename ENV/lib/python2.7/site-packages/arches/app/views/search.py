    ...
    if term_filter != '':
        for index, select_box in enumerate(JSONDeserializer().deserialize(term_filter)):
            ...
            if groupid:
                ...
                logging.warning('FULL GROUP QUERY - %s', JSONSerializer().serialize(group_query, indent=4))
                
                # logging.warning('FULL_NESTED_QUERY - %s', JSONSerializer().serialize(group_query, indent=4));
                # logging.warning('NESTED_QUERY - %s', json.dumps(group_query, indent=4));
                
                selectbox_boolfilter.must(group_query)
                
                logging.warning('BOX QUERY - %s', JSONSerializer().serialize(selectbox_boolfilter, indent=4))
...
def nested_query_from_pathed_values(pathed_values, stem):
    ...
    # logging.warning('PATHED_VALUES for %s - %s', stem, JSONSerializer().serialize(pathed_values, indent=2))
    
    logging.warning("ITERATING ANOTHER LEVEL DEEP. STEM = %s", stem)
    ...
    for leaf_group in leaf_groups:
        ...
        logging.warning('leaf_group: %s', leaf_group)
        
        bool_term.must( Terms(field=stem+'.conceptid', terms=leaf_group['val']))
    ...
    # nestpath = stem + '.child_entities'
    # logging.warning("CREATING NESTED TERM WITH PATH = %s", nestpath)
    nested_term = Nested(path=stem, query=bool_term)
    
    # logging.warning('INTERMEDIATE NESTED TERM - %s', JSONSerializer().serialize(nested_term, indent=4))
    logging.warning('_________')
    return nested_term;
