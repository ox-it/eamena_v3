...
#    Must go back to this method once new Automatic Resource ID has been fully developed (AZ 10/08/16) Update 06/09/16: EAMENA_ID.E42 now used as sorting criterion.
    ...
    if not 'year_min_max' in temporal_filter: #this is a user generated search
        for index, time_filter in enumerate(temporal_filter):
            logging.warning('------time_filter %s', time_filter)
            if len(time_filter['year_min_max']) != 0:
                is_empty_temporal_filter = False
            if len(time_filter['filters']) != 0:
                is_empty_temporal_filter = False
    # if this is a search for related resources there is no need to set is_empty_temporal_filter to False
            ...
            if not groupid == 'No group':
                ...
                for term in select_box:
                    ...
                    if term['type'] == 'term':
                        ...
                        boolfilter_folded.should(Match(field='child_entities.value', query=term['value'], type='phrase_prefix', fuzziness='AUTO'))
                        boolfilter_folded.should(Match(field='child_entities.value.folded', query=term['value'], type='phrase_prefix', fuzziness='AUTO'))
                        boolfilter_folded.should(Match(field='child_entities.value.folded', query=term['value'], fuzziness='AUTO'))
                        nested = Nested(path='child_entities', query=boolfilter_folded)
                        boolfilter_folded2.should(Match(field='domains.label', query=term['value'], type='phrase_prefix', fuzziness='AUTO'))
                        boolfilter_folded2.should(Match(field='domains.label.folded', query=term['value'], type='phrase_prefix', fuzziness='AUTO'))
                        boolfilter_folded2.should(Match(field='domains.label.folded', query=term['value'], fuzziness='AUTO'))
