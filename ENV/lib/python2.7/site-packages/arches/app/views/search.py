...
#    Must go back to this method once new Automatic Resource ID has been fully developed (AZ 10/08/16) Update 06/09/16: EAMENA_ID.E42 now used as sorting criterion.
    ...
    filter_resource_types = JSONDeserializer().deserialize(request.GET.get('resourceTypes', ''))
    ...
    for resource_type in filter_resource_types:
        boolfilter_nested = Bool()
        boolfilter_nested.must(Terms(field='nested_entity.entitytypeid', terms=[resource_type]))
        nested = Nested(path='nested_entity', query=boolfilter_nested)
