...
def search_terms(request):
    ...
    for result in results['hits']['hits']:
        ...
        if group_root_node != 'None':
            ...
            res = Entity().get_mapping_schema_to_root(entity_type)
            # search parents for group_root_node
            delete_result = True
            for parent in res[entity_type]['steps']:
        ...
        for index, select_box in enumerate(JSONDeserializer().deserialize(term_filter)):
            ...
            if groupid:
                ...
                for term in select_box:
                    ...
                    if term['type'] in ['concept']:
                        
                        term_parent_concept = Concept.get_parent_concept(term['value'])
                        # logging.warning('TERM PARENT CONCEPT - %s', JSONSerializer().serialize(term_parent_concept))


                        path = {}
                        
                        to_group_mappings = models.Mappings.objects.filter(entitytypeidto=groupid)

                        # # strip the text from the start of the id - it is there in error atm
                        # termid = term['id'][len(term['text']):]
                        # to_term_mappings = models.Mappings.objects.filter(entitytypeidto=term_parent_concept.legacyoid)

                        term_schema = Entity.get_mapping_schema_to(term_parent_concept.legacyoid)
                        # logging.warning('TERM SCHEMA - %s', JSONSerializer().serialize(term_schema))
                        
                        term_path = term_schema['HERITAGE_RESOURCE_GROUP.E27']['steps'];
                        # logging.warning('FULL TERM PATH - %s', json.dumps(term_path, indent=4))
                        #this path begins at the root, and ends up at the node in question
                        
                        # truncate the path so that it begins at the group root
                        # term_path_from_group = []
                        # found = False
                        # for step in term_path:
                        #     if step['entitytypedomain'] == groupid:
                        #         found = True
                        #     if found:
                        #         term_path_from_group.append(step)
                        
                        # logging.warning('TERM PATH FROM GROUP - %s', json.dumps(term_path_from_group, indent=4))
                        term_paths[term['value']] = term_path
                        
                        # logging.warning('GROUP MAPPINGS - %s', JSONSerializer().serialize(to_group_mappings))
                        # logging.warning('TERM MAPPINGS - %s', JSONSerializer().serialize(to_term_mappings))
                        # logging.warning('TERM PATH - %s', JSONSerializer().serialize(path))
                        logging.warning('...........')
                ...
                # logging.warning('TERM PATHS - %s', json.dumps(term_paths, indent=4))
                # combine the traced path to build a nested query
                
                group_query = nested_query_from_paths(term_paths)
...
def nested_query_from_pathed_values(pathed_values, stem):
    """
    Given a dict of pathed values to query terms from the root (keyed by value), return a nested query
    pathed_values: e.g. 
    [
        {
            val: '29430-4955-...'
    ...
    for leaf_groups as leaf_group:
        for leaf_group as leaf_value:
            # bool_term.must( something )
            # bool_term.must( 
            #     {
            #         'terms': {
            #             path + conceptid : value['val']
            #          }
            #      }
            # )
            bool_term.must( Terms(field=path+.conceptid, terms=value['val']))
    ...
    for key in branch_groups:
        ...
        nested_bool = Bool()
        nested_term = Nested(path=path_'.conceptid', query=nested_bool)
        
        #remove head from each path and recurse
        for value in branch_group:
            value['path'] = value['path'][1:]
        sub_query = nested_query_from_pathed_values(branch_group, path + '.child_entities')
        
        nested_bool.must(sub_query)
    ...
    #     logging.warning('PATH - %s', json.dumps(path, indent=2))
    #     if len(path) > 0:
    #         # add this to the group sharing the same first step
    #         head_step = path[0]
    #         head_id = head_step['entitytypedomain']
    #         if head_id not in groups:
    #             groups[head_id] = []
    #         groups[head_id].append(path)
    #     else:
    #         #single query - simply add the term as a must condition (with appropriate key for nesting depth e.g. 'nested_entity.child_entities.child_entities.conceptid')
    #         
    #         pass
    # 
    # # combine results of recursion with a boolean
    # 
    # # return nested( bool ( must (    [nested_query_from_paths(g) for g in groups]   )))
    # 
    # for group in groups:
    #     if len(group) > 1:
...
def group_paths(paths):
    """
    group the given dict of paths such that each list of 1 is its own group, and there is a group for each unique head, if there is also a tail
    """
    singleton_groups = {}
    tail_groups = {}
