...
def index_resources_into_map_layers():
    
    result_summary = {}
    se = SearchEngineFactory().create()
    
    se.delete_index(index='maplayers')
    ...
    index_resources_by_type_to_maplayers(resource_types, result_summary)
    

def index_resources_by_type_to_maplayers(resource_types, result_summary):
    """
    Collects and indexes all resources into the maplayers index only
    """

    for resource_type in resource_types:
        resources = archesmodels.Entities.objects.filter(entitytypeid = resource_type)
        print "Indexing {0} {1} resources into maplayers".format(len(resources), resource_type[0])
        result_summary[resource_type[0]] = {'database':len(resources), 'indexed':0}
        errors = []
        for resource in resources:
            try:
                resource = Resource().get(resource.entityid)
                resource.index_into_map_layers()
