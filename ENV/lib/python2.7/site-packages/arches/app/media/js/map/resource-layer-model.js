    ...
    return function(config, featureCallback) {
        ...
        var layer = function () {
            ...
            var clusterStyle = function(feature, resolution) {
                var isOldCluster = feature.get('features');
                if(isOldCluster) {
                    //this is a cluster from an openlayers cluster source
                    var size = feature.get('features').length;
                } else {
                    //this is a cluster originating from supercluster
                    if(feature.get('point_count')) {
                        var size = feature.get('point_count');
                    } else {
                        //supercluster has returned the original feature (i.e. cluster of 1)
                        var size = 1;
                    }
                }
                var mouseOver = feature.get('mouseover');
