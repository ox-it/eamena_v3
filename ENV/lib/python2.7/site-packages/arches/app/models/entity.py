...
class Entity(object):
    ...
    def get_mapping_schema_to(cls, entitytypeid):
        """
        Gets the partial schema of *incoming* connections to the given entity type id, and their steps
        """
        # logging.warning("entity type id = %s", entitytypeid)
        ret = {}
        mappings = archesmodels.Mappings.objects.filter(entitytypeidto = entitytypeid)
        # mappings = archesmodels.Mappings.objects.filter(entitytypeidfrom = entitytypeid)
        
        logging.warning('get_mapping_schema MAPPINGS - %s', JSONSerializer().serialize(mappings))
        for mapping in mappings:
    ...
    @classmethod
    def get_mapping_schema_to_root(cls, entitytypeid):
        """
        Gets a complete entity schema graph to root, for a single entity type given an entity type id
        """
