...
class Resource(Entity):
    ...
    def index_into_map_layers(self):
        """
        As per above function, but only concerned with the maplyers index so that reindexing can be done more quickly
        """
        se = SearchEngineFactory().create()
    ...
    def prepare_maplayer_index(self, create=False):
        ...
        for resource_type in settings.MAPLAYERS_INDEX_RESOURCE_TYPES:
            index_settings['mappings'][resource_type] = {
                "properties": {
                    "geometry": {
                        "type": "geo_shape"
                    }
